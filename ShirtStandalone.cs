...
public class ShirtStandalone : EditorWindow
{
	...
	private static string path, customNameString = ""; //url = "https://cdn.discordapp.com/attachments/296241012483817473/411961984620167168/Shirt.unitypackage";
	...
	void OnEnable()
	{
		...
		Texture icon = AssetDatabase.LoadAssetAtPath<Texture> ("Assets/ShirtCreator/Icons/zombie.png");
		if (AssetDatabase.IsValidFolder ("Assets/UMC_Assets")) {
		} else {
			AssetDatabase.ImportPackage ("Assets/ShirtCreator/Shirt.unitypackage", importdialog);
		}
	...
	void OnGUI()
	{
		...
		if (GUILayout.Button ("Select", GUILayout.Width (200))) 
		{
			...
			if (path.Length != 0)
			{
				...
				AssetDatabase.ImportAsset ("Assets/UMC_Assets/Resources/Bundles/Items/Shirts/Hoodie_Orange/Material.mat");
		...
		*/
		...
		EditorGUILayout.HelpBox ("This option allows your project to be automatically set up for the next item, i.e removing any objects currently in scene and deleting the UMC_Assets file. Don't use this if you plan on making more shirts after this!", MessageType.Info);
		if (GUILayout.Button ("Create Mod Files", GUILayout.Width(350))) 
		{
			 ...
			 * */
				...
				//This is the new weird and wonky Unity 5+ asset bundling system. While I've never coded in Unity 4, this system definitely seems more complex. The function below gets the path to the UMC_Assets folder and marks it as a "unity3d" bundle. This removes the need for user based renaming.
				AssetImporter.GetAtPath ("Assets/UMC_Assets").SetAssetBundleNameAndVariant ("spaghett", "unity3d");
				...
				System.IO.Directory.CreateDirectory (path + "/" + itemNameFlat + "/test");
				System.IO.Directory.Delete (path + "/" + itemNameFlat + "/test", true);
				//Checking whether or not the user enabled creation of .dat files
				if (generateDats) {
					using (StreamWriter sw = File.CreateText (path + "/" + itemNameFlat + ".dat")) {
					...
					using (StreamWriter sx = File.CreateText (path + "/English.dat")) {
				...
				*/
				...
				UnityEditor.AssetDatabase.Refresh ();
				Debug.Log (path);
				File.Move (path + "/spaghett.unity3d", path + "/" + itemNameFlat + "/" + itemNameFlat + ".unity3d");
				File.Move (path + "/English.dat", path + "/" + itemNameFlat + "/English.dat");
				File.Move (path + "/" + itemNameFlat + ".dat", path + "/" + itemNameFlat + "/" + itemNameFlat + ".dat");
				File.Delete (path + "/Desktop");
				File.Delete (path + "/Desktop.manifest");
				if (!KeepManifests)
					File.Delete (path + "/" + itemNameFlat + ".manifest");
			}
		}
		if (generateDats == false && customNameString == "" || itemName == "" && customNameString == "") {
		...
		if (devFold) {
			...
			if (GUILayout.Button ("Reset Scene", GUILayout.Width (150))) {
				AssetDatabase.DeleteAsset ("Assets/UMC_Assets");
	...
	void OnInspectorUpdate()
	{
		...
		if (AssetDatabase.IsValidFolder ("Assets/UMC_Assets")) {
			ShirtTex = AssetDatabase.LoadAssetAtPath<Texture2D> ("Assets/UMC_Assets/Resources/Bundles/Items/Shirts/Hoodie_Orange/Shirt.png");
			ShirtEm = AssetDatabase.LoadAssetAtPath<Texture2D> ("Assets/UMC_Assets/Resources/Bundles/Items/Shirts/Hoodie_Orange/Emission.png");
			...
			ShirtPreview = AssetDatabase.LoadAssetAtPath<Texture2D> ("Assets/ShirtCreator/Icons/Null.png");
		 ...
		 * 
		if (AssetDatabase.IsValidFolder ("Assets/UMC_Assets")) {
			UnityEngine.Object prefab = AssetDatabase.LoadAssetAtPath ("Assets/UMC_Assets/Resources/Bundles/Items/Shirts/Hoodie_Orange/Item.prefab", typeof(GameObject));
	...
	void OnInspectorUpdate()
	{
		...
		if (AssetDatabase.IsValidFolder ("Assets/UMC_Assets")) {
			mat = AssetDatabase.LoadAssetAtPath<Material> ("Assets/UMC_Assets/Resources/Bundles/Items/Shirts/Hoodie_Orange/Material.mat");
			ShirtTex = AssetDatabase.LoadAssetAtPath<Texture2D> ("Assets/UMC_Assets/Resources/Bundles/Items/Shirts/Hoodie_Orange/Shirt.png");
			ShirtEm = AssetDatabase.LoadAssetAtPath<Texture2D> ("Assets/UMC_Assets/Resources/Bundles/Items/Shirts/Hoodie_Orange/Emission.png");
			ShirtMet = AssetDatabase.LoadAssetAtPath<Texture2D> ("Assets/UMC_Assets/Resources/Bundles/Items/Shirts/Hoodie_Orange/Metallic.png");
		...
		else if (!AssetDatabase.IsValidFolder ("Assets/UMC_Assets")){
			ShirtPreview = AssetDatabase.LoadAssetAtPath<Texture2D> ("Assets/ShirtCreator/Icons/Null.png");
		}
		if (AssetDatabase.IsValidFolder ("Assets/UMC_Assets") && hasRan == false) 
		{
			mat.SetColor ("_EmissionColor", Color.black);
			mat.SetTexture ("_MetallicGlossMap", null);
			hasRan = true;
	...
	private void bundleAssets()
	{
		...
		if(path.Length > 0)
		{
			...
			//if(!BuildPipeline.BuildAssetBundle(AssetDatabase.LoadMainAssetAtPath("Assets/UMC_Assets",typeof(UnityEngine.Object)), AssetDatabase.LoadAllAssetsAtPath("Assets/UMC_Assets"), path, BuildAssetBundleOptions.UncompressedAssetBundle, BuildTarget.StandaloneWindows));
